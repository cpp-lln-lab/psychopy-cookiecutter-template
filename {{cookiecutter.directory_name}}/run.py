...
fmri_setting = {"trigger": "5", "dummy_vol": 3, "tr": 3, "slice_per_vol": 60}
...
def run_experiment(config, trials):
    ...
    startexp = instructions(
        window=Experiment.window, instruction_txt=exp_txt, color="white"
    )
    endexp = instructions(
        window=Experiment.window, instruction_txt=end_txt, color="white"
    )
